...
- [x] Página `/perfil/:slug` renderiza dados do `userProfile` via GROQ
- [x] ISR com webhook do Sanity revalida no publish/update
- [ ] Edição de perfil (bio, skills, links, avatar) apenas autenticada (server-side)
- [x] Componentes UI base funcionais (`Avatar`, `Tag`, `Card`, `Section`)
...
- [x] Página pública de perfil com ISR
- [ ] CRUD de perfil autenticado
- [x] Webhook de revalidação
- [ ] Testes unitários > 80%
